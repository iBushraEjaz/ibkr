...
def _upsert_position(symbol: str, shares: int, entry_price: float, current_price: float, stop: float):
    ...
    with SessionLocal() as db:
        ...
        if pos:
            ...
            db.add(Position(symbol=symbol, shares=shares, entry_price=entry_price,
                            current_price=current_price, stop=stop, pnl=pnl))
        db.commit()
...
def _patch_strategy(strategy: ORBStrategy):
    ...
    def on_entry_filled(trade):
        ...
        _upsert_position(strategy.symbol, strategy.shares, fill_px, fill_px, strategy.current_stop)
    ...
    try:
        ...
        while True:
            ...
            if now.hour >= 16:
                ...
                break
            if all(s.state == "done" for s in strategies):
